...
class MLPBC(nn.Module):
    ...
    def __init__(
        self, config: TrainConfig, hidden_size: int, n_layer: int = 4, dropout=0.1
    ) -> None:
        super().__init__()
        ...
        self.main_stick_head = nn.Linear(
            hidden_size, embed_config.num_main_stick_clusters
        )
...
Arch.register(
    "MLPDebug-512-8", make_net=MLPDebug, hidden_size=512, n_layer=8, dropout=0.0
)
