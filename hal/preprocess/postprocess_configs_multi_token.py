...
def sample_main_stick_fine(
    pred_C: TensorDict, temperature: float = 1.0, frame: int = 1
) -> tuple[float, float]:
    main_stick_probs = torch.softmax(
        pred_C[f"main_stick_{frame}"] / temperature, dim=-1
    )
...
def predict_with_delay(frame: int) -> PostprocessConfig:
    return PostprocessConfig(
        transformation_by_controller_input={
            k: partial(v, frame=frame)
            for k, v in BASELINE_TRANSFORMATION_BY_CONTROLLER_INPUT.items()
        }
    )
