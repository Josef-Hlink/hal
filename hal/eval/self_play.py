...
def gpu_worker(
    ...
    context_window_BPL: TensorDict = torch.stack(
        [shared_batched_model_input_BP for _ in range(seq_len)], dim=-1
    ).to(device)  # type: ignore
    B, P, L = context_window_BPL.shape
    logger.info(
        f"Context window shape: {context_window_BPL.shape}, device: {context_window_BPL.device}"
    )
    ...
    while not all(event.is_set() for event in stop_events):
        ...
        if iteration < seq_len:
            ...
            context_window_BPL[..., iteration].copy_(
                shared_batched_model_input_BP, non_blocking=True
            )
        else:
...
def run_closed_loop_evaluation(
    ...
    shared_batched_model_input_BP: TensorDict = torch.stack(
        [mock_model_inputs_P for _ in range(n_workers)],
        dim=0,  # type: ignore
    )
    ...
    shared_batched_model_output_BP: TensorDict = torch.stack(
        [preprocessor.mock_preds_as_tensordict() for _ in range(n_workers * 2)],
        dim=0,  # type: ignore
    ).view(n_workers, 2)
    shared_batched_model_output_BP = share_and_pin_memory(
        shared_batched_model_output_BP
    )
